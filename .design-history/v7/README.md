...
- Shop section with an honest coming-soon state.
- Validated inquiry form that prepares a reviewable email draft; direct email and Instagram links.
...
The inquiry form creates a mailto draft and a copyable summary. It does not send, store, or log inquiries, reserve dates, or connect to a calendar. Visitors must send the draft through their email application. Before launch, decide whether to keep email inquiries or connect a form service and a scheduling provider.

The shop has no invented products, prices, checkout, or payment processing. Add confirmed products, fulfillment details, and a checkout provider before opening sales. Add motion work when the chosen reel and film credits are available.
...
- `dist/app.js`: project galleries, inquiry draft, motion.
...
`dist/palette.css` adds a shared vermilion accent to both pages: `#ff5c35` on dark backgrounds, `#bd361b` on light backgrounds, and `#fae8df` for the Studio inquiry surface. Navigation details, section labels, project arrows, contact links, and buttons share these colors.
